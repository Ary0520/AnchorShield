...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: { name: string; pct: number | null }; value: number }> }) {
  ...
  return (
    ...
    >
      ...
      {d.pct !== null ? (
        <>
          <p style={{ color: riskColor(d.pct) }}>{d.pct.toFixed(2)}% implied risk</p>
          <p style={{ color: "rgba(255,255,255,0.4)" }}>YES price ÷ 100 = probability</p>
        </>
      ) : (
...
export default function RiskCurvePage() {
  ...
  // Bar chart data — one bar per asset (no USDT since it's not on Stellar)
  const chartData = markets
    .filter((m) => m.asset !== "USDT")
    .map((m) => ({
      name: m.asset,
      pct: m.impliedPct,
      value: m.impliedPct ?? 0,
      color: riskColor(m.impliedPct),
    }));
    ...
    <div className="p-6 space-y-6 min-h-full" style={{ background: "#0a0a12" }}>
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-white font-semibold text-sm">LIVE RISK SIGNAL</p>
            <p className="text-xs mt-0.5 flex items-center gap-1.5" style={{ color: "rgba(255,255,255,0.35)" }}>
              <Info size={11} />
              Market-implied depeg probability · Reflector oracle · 5 min updates
            </p>
          </div>
          <div className="relative">
            ...
            {showTooltip && (
              <div
                className="absolute right-0 top-8 z-10 text-xs p-3 rounded-lg w-64"
                style={{
                  background: "#1a1a2e",
                  border: "1px solid rgba(255,255,255,0.1)",
              ...
              >
                YES price ÷ 100 = implied probability. Higher = market expects more risk for that stablecoin.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="h-52">
          {loading ? (
            <div className="h-full flex items-end gap-6 px-8 pb-8">
              {["USDC","EURC","PYUSD","MGUSD"].map((a) => (
                <div key={a} className="flex-1 flex flex-col items-center gap-2">
                  <div className="w-full rounded-t animate-pulse" style={{ height: "40%", background: "rgba(255,255,255,0.05)" }} />
                  <p className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>{a}</p>
                </div>
              ))}
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 16, right: 16, left: 0, bottom: 8 }} barCategoryGap="35%">
                ...
                <ReferenceLine y={0.5} stroke="rgba(245,158,11,0.25)" strokeDasharray="4 4" />
                <ReferenceLine y={2}   stroke="rgba(239,68,68,0.25)"   strokeDasharray="4 4" />
        ...
        {/* Legend */}
        <div className="flex items-center gap-5 mt-2" style={{ color: "rgba(255,255,255,0.3)" }}>
          {[
            { color: "#22c55e", label: "Low (<0.5%)" },
            { color: "#f59e0b", label: "Moderate (0.5–2%)" },
            { color: "#ef4444", label: "Elevated (>2%)" },
          ].map((l) => (
            <div key={l.label} className="flex items-center gap-1.5 text-xs">
              <span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ background: l.color, opacity: 0.8 }} />
              {l.label}
            </div>
          ))}
        </div>
